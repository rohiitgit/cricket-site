        ...
        <div className="container mx-auto relative w-full h-screen overflow-hidden">
            ...
            <div className="row absolute inset-0">
                {itemsData.map((item, index) => (
                    <div
                        key={index}
                        className={`slider-item absolute inset-0 bg-center bg-cover transition-opacity duration-500 ${index === currentIndex ? "active" : ""
                            }`}
            ...
            <div className="preview-images absolute right-4 top-1/2 transform -translate-y-1/2 flex space-x-4">
                {getPreviewImages().map((item, index) => (
                    <div
                        key={index}
                        className={`h-40 w-24 bg-center bg-cover rounded-md overflow-hidden ${animatePreview && index === 0 ? "animate-preview" : ""
                            }`}
                        style={{ backgroundImage: `url(${item.imageUrl})` }}
                    >
                        <img
